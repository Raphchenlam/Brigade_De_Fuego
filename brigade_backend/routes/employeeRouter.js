...
router.put('/newPassword/:employeeNumber',
    passport.authenticate('basic', { session: false }),
    (req, res, next) => {

        const employee = req.user;

        if (!employee) return next(new HttpError(401, "Connexion nécessaire"));

        const employeeNumberParams = req.params.employeeNumber;
        if (!employeeNumberParams || employeeNumberParams == '') return next(new HttpError(400, 'Le champ employeeNumber est requis'));
        if (!regex.validEmployeeNumber.test(employeeNumberParams)) return next(new HttpError(422, 'Le numéro d\'employé ne respecte pas les critères d\'acceptation'));
        if (employee.employeeNumber != employeeNumberParams) return next(new HttpError(403, 'Vous n\'avez pas l\'authorisation de modifier le mot de passe d\'un autre employé'));

        employeeQueries.selectEmployeeByEmployeeNumber(employeeNumberParams).then(employee => {
            if (employee) {
                ...
                crypto.pbkdf2(password, passwordSalt, 100000, 64, "sha512", async (err, derivedKey) => {
                    ...
                    try {
                        const employeeAccountWithPasswordHash = await employeeQueries.updateNewEmployeePassword(employeeNumberParams, passwordSalt, passwordHashBase64);
                        res.json(employeeAccountWithPasswordHash);
                    } catch (err) {
                        return next(err);
                    }
                ...
                return next(new HttpError(404, `Employé avec le numéro d'employé ${employeeNumberParams} inexistant ou introuvable`));
            }
        })
    });
